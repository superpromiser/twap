...
function Trade() {
  ...
  const [buyOrSellAmount, setBuyOrSellAmount] = useState(0);
  ...
  const CreateAdvanced = async () => {
    if (!wallet.address) {
      ...
      const pairAddress = await factoryV2.getPair(
        addresses[chainID].token1,
        addresses[chainID].token2
      );
      // console.log(pairAddress);
      // console.log(tokenContract, factoryV2, routerV2);

      const uniswapV2OracleFactory = await getV2OracleFactory(
        provider.getSigner()
      );

      // let dration = Number(durationIn) * 1000;
      ...
      // console.log(uniswapV2Oracle.address);
    ...
    if (type === "buy") {
      ...
      await routerV2
        ...
        .swapExactTokensForETH(
          buyAmount,
          0,
          [addresses[chainID].token1, addresses[chainID].token2],
          account,
      ...
      await routerV2
        ...
        .swapExactETHForTokens(
          0,
          [addresses[chainID].token2, addresses[chainID].token1],
          account,
          ethers.constants.MaxUint256,
          {
    ...
    <StyledContainer>
      ...
      <Box position={"relative"}>
        <TradeBox>
          ...
          {twapCreated && (
            <>
              <TradeHead>Twap Started</TradeHead>
          ...
          <AdvancedBox onClick={() => setAdvanced(!advanced)}>
            {!advanced ? (
